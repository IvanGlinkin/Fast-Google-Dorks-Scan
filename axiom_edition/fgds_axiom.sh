...
function Query {
	...
	for start in `seq 0 10 40`; ##### Last number - quantity of possible answers
		do
			...
			#query=$(echo; curl -sS -b "CONSENT=YES+srp.gws-20231129-1-RC1.es+FX+331" -A "\"$randomuseragent\"" "https://www.google.com/search?q=$gsite%20$1&start=$start&client=firefox-b-e")
			query=$(echo; curl -sS -A "$randomuseragent" "https://www.google.com/search?q=$gsite%20$1&start=$start&client=firefox-b-e")
